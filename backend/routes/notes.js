...
router.post('/', requireMember, async (req, res) => {
  try {
    ...
    if (req.user.tenant_plan === 'free') {
      const countResult = await pool.query('SELECT COUNT(*) as count FROM notes WHERE tenant_id = $1', [req.user.tenant_id]);
      if (parseInt(countResult.rows[0].count) >= 3) {
        return res.status(403).json({ error: 'Free plan limit reached. Maximum 3 notes allowed.', code: 'PLAN_LIMIT_REACHED' });
      }
    }
...
router.delete('/:id', requireMember, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query('DELETE FROM notes WHERE id = $1 AND tenant_id = $2 RETURNING id', [id, req.user.tenant_id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Note not found' });
